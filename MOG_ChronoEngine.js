...
const TOOL_POSITION_DEFAULT = 0;
const TOOL_POSITION_TARGET = 1;
const TOOL_POSITION_USER = 2;
const TOOL_POSITION_SKILL_TARGET = 3;
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_ChronoEngine');
	...
	Moghunter.ras_maxBattleMembers = Number(Moghunter.parameters['Max Battle Members'] || 4);
	...
	Moghunter.ras_cursorActorR = Number(Moghunter.parameters['Cursor Actor Rotation'] || 0.00); 
	Moghunter.ras_cursorActorB = Number(Moghunter.parameters['Cursor Actor Blend Mode'] || 0); 	
	Moghunter.ras_cursorActorO = Number(Moghunter.parameters['Cursor Actor Opacity'] || 160); 	
...
//==============================
// * set Chrono Temp - FlintX added lines here to fit new "bossvictory, draw and threat cleared" phases.
//==============================
Game_Temp.prototype.setChronoTemp = function() {
	...
	this._chrono.bossvictory = [false,false,0]; // Added for Boss Victory
	this._chrono.eventvictory = [false,false,0];
	this._chrono.draw = [false,false,0]; //Added for Draw Phase
	this._chrono.killminions = [false,false,0, []]; // Added for Kill Minions Phase
	this._chrono.threatcleared = [false,false,0]; //Added for Threat Cleared Phase
...
//==============================
// * Should keep dead body - FlintX's brother added for the new "keep dead" body notetag.
//==============================
//This notetag allows you to keep enemy dead bodies on map instead of having them dissapearing as part of the collapse function.
//Useful in case you want to keep their sprites on screen to run a map event cutscene or something after the battle.
Game_Battler.prototype.shouldKeepDeadBody = function() {
	return this._ras._KeepDeadBody === true;
};
...
//==============================
// * tool Sys Init Battler - FlintX tiny game design suggestion
//==============================
Game_Battler.prototype.toolSysInitBattler = function() {
	...
	this._ras.guard.chronoPer = 100; //Since you cannot predict exactly which character the enemies are going to attack in Chrono Mode, I would suggest changing this value to make guard negate 100% of damage instead. This way, selecting the "Guard Command" would turn into a gamble/guess game that would reward you with the chance of complete nullify the damage of an incoming attack. Well, at least this sounds more attractive than wasting your character turn trying to defend an attack that is not going to hit him. (Probably one of the reasons why Chrono Trigger doesn't have a guard command in ther first place).
...
//=============================================================================
// ** Game Actor setup? - FlintX: This is an incomplete implementation.  
//=============================================================================
//The ideia behind this is to make events check for the actor_id notetag comment which would make said event be treated like an Actor. (Just like MOG enemy_id works).
//And later temporarily make the event load as a controllable player character in battles. *OBS: Something like a "guest" party member system or an temporary ally, for creating special battle encounters and stuff.

//==============================
// * setup
//==============================
/*var _mog_toolSys_gactor_setup = Game_Actor.prototype.setup;
Game_Actor.prototype.setup = function(actorId, x, y) {
	_mog_toolSys_gactor_setup.call(this,actorId, x, y);
    this.loadToolSysNotes()
};*/
...
//==============================
// * Execute Collapse - FlintX edit
//==============================
Game_CharacterBase.prototype.executeCollapse = function(char,battler) {
	char._user.collapse = [true, battler.shouldKeepDeadBody() ? 0 : 90]; //added here for the new "keep dead body" notetag.
...
Game_CharacterBase.prototype.prepareDeadSprite = function(char,battler) {
	char._through = true;
    char._walkAnime = true;
    char._stepAnime = true;
...
Game_CharacterBase.prototype.updateRasBattler = function() {
	...
	if ($gameSystem.isChronoMode()) {this.updateChronoModeChar()} //the call for the new stepAnime function
};
...
//==============================
// * update Chrono Mode Char - FlintX curiosity notes
//==============================
Game_CharacterBase.prototype.updateChronoModeChar = function() {
	if (!$gameSystem._chronoMode.inTurn) {
		this._stepAnime = this.canSpepAnimeCN(); //I might just be talking nonsense here, but it seems that this line overwrites the standard stepAnime function and replaces it with this new one, meant for working exclusively within Chrono Mode. Which corresponds to its several "phases", Going from the process of initiating a fight, to all the way till the very end, after victory poses and battle results screens.
	}; //However, since it turns the ~standard stepAnime into this... new function?~, I couldn't turn stepAnime ON/OFF directly on the phases the way I wanted by the standard method anymore. And I couldn't figure out a way to switch this new function ON/OFF inside the phases code either. So I had to use this "workaround" on the code bellow.
}; 	//I dunno why MOG wanted this to work like that, but for my testings this and the bellow function can complete be deleted from the code without causing anything to break at all. And so keep the standard stepAnime class behavior (I think? I did a bit of testing, but not enough to confirm this 100%). However, doing so would take a lot more work, since you would have to turn the stepAnime ON/OFF manually in each of the ChronoMode.phases.  

//==============================
// * can Step Anime CN - FlintX edited to remove idle step animations from battle preparation, and after the battle ends (a workaround, since this function changes entirely how the stepANime class triggers I guess. I cannot code, sorry).
//==============================
Game_CharacterBase.prototype.canSpepAnimeCN = function() { 
	//Added to get rid of that stupid step animation that happens just before the battles starts. So that the characters doesn't look stupid doing moonwalks while standing still. I'm not sure if this is the correct way of doing this, or if it breaks anything in the code, but everything seems alright on my testings so far. Characters still have step anime when walking though, probably due to Update Force Movement Ras code or something directly from MOG_CharPoses.
	//The real ideia behind this was to turn off step animations for all ChronoMode phases except for the ones I wanted to keep them. But since I'm dumb and couldn't figure out a way to simplify all that into a sigle string, without having to repeat the class prefix, I made it like that. And it works, so... (Again, I'm no coder. Just trying to make things work the way I want here).
	const old = false;
	if (!old) {
		const phases = [
			2, //this value is to keep the step animation for the new "Draw" phase.
			3, //this value is to keep step animations for all the actions that happens during the actual combat.
			5, //this value is to keep the step animation for the "Victory pose". But it doesn't affect the stepAnime after the victory pose takes places. See "Update Victory" section for this one.
			8, //this value is to keep step animations for the new "Sheath phase" pose that happens just before Victory Phase.
			12 //this value is to keep the Sheath pose step animations for the new "Event Fight" phase.
		];
		if (!phases.includes($gameSystem._chronoMode.phase)) {return false}; 
	/*} else {
		if ($gameSystem._chronoMode.phase != 2 && //this value is to keep the step animation only for the new "Draw" phase.
			$gameSystem._chronoMode.phase != 3 && //this value is to keep step animations for all the actions that happens during the actual combat.
			$gameSystem._chronoMode.phase != 5 && //this value is to keep the step animation only for the "Victory pose". But it doesn't affect the stepAnime after the victory pose takes places. See "Update Victory" section for this one.
			$gameSystem._chronoMode.phase != 8 && //this value is to keep step animations only for the new "Sheath phase" pose that happens just before Victory Phase.
			$gameSystem._chronoMode.phase != 12) {return false};
	}*/
	if (!this.battler().canMove()) {return false};
    return true;
	}
	       ...
	       if (l.code === 108) {var comment = l.parameters[0].split(' : ')
				...
				if (comment[0].toLowerCase() == "enemy_id"){
					...
					if ($dataEnemies[enemyId]) {
					   ...
					   /*if (comment[0].toLowerCase() == "actor_id"){  //Atempt to replicate "enemy_id" to make events reads this notetage and behave like an game Actor, and temporarily join the player party as a controllable allied battler in fight. This is just an incomplete ideia.
						var actorId = Number(comment[1]);
						if ($dataActors[actorId]) {
						   if (!this._user.battler) {this._this._actor = new Game_Actor(actorId, 0, 0)};
						   if (this._poses) {
							   this._poses.enabled = true
						   }
						}
...
//==============================
// * clear Base Parameters - FlintX added here (not really sure if this is even necessary in the first place for those phases, since one of them plays before the battle starts and the other just before the victory phase, which already does that. But anyways)
//==============================
Game_Chrono.prototype.clearBaseParameters = function() {
	...
	$gameTemp._chrono.draw = [false,false,0]; //Added for Draw phase
	$gameTemp._chrono.threatcleared = [false,false,0]; //Added for Threat Cleared phase
...
//==============================
// * Battle Start Chrono - FlintX edited values and added here
//==============================
Game_Chrono.prototype.battleStartChrono = function() {
	BattleManager.playBattleBgm();
	$gameSystem._chronoMode.phase = 2; //Modified to fit the new Draw phase
	...
	this.executeDraw(); //Added for the new Draw phase.
};
...
//==============================
// * is Event Fight - FlintX added for the new "event fight" notetag.
//==============================
//I added this to make fights that will skip the battle results screen, "Victory phase" and everything that comes with it, like XP and Gold gain. All in order to have more flexibility to run map events after the battles.
Game_Chrono.prototype.isEventFight = function() {
	var isEventFight = false;
	const enemies = $gameMap.enemiesF();
	for (var enemy_index = 0; enemy_index < enemies.length; enemy_index++) {
		const battler = enemies[enemy_index].battler();
		if (battler._ras._eventfight === true) {
			isEventFight = true;
			break;
		}
	}
...
//==============================
// * Execute Draw Phase - FlintX added for Draw phase
//==============================
Game_Chrono.prototype.executeDraw = function() {
	$gameSystem._chronoMode.phase = 2; //This value is tied to the "draw pose" code on my custom MOG_CharPoses.js
    $gameTemp._chrono.phase[0] = true;
	$gameTemp._chrono.phase[1] = 3;	
	$gameSystem._chronoMode.phaseDuration = 44; //Duration of draw pose animation. This particular time was set for a 6 frames long animation.
AudioManager.playSe({name: "battleStartCT", pan: 0, pitch: 100, volume: 100});
};

//==============================
// * update Draw - FlintX added with someone else help (Because I don't understand what half of those codes means or are doing)
//==============================
Game_Chrono.prototype.updateDraw = function() {
    $gameTemp._chrono.phase[0] = false;
	$gameTemp._chrono.phase[1] = 3;	
	if ($gameTemp._chrono.draw[2] === 0) {
		
		 //Of course we need audio for Draw phase! The audio here will execute at the end of the pose animation. If moved to the "executeDraw" function or to the pose code itself in MOGCharPoses it probably will play right at the start of the pose animation. Your call.
		$gameTemp._chrono.draw[2] = 1;
		$gameTemp._chrono.draw[0] = true;
		$gameTemp._chrono.draw[1] = true;
		$gameSystem._chronoMode.phaseDuration = 7; //Not sure what this value does, but I think it works as a delay before the ATB gaugue shows up and the actual fight starts. However, since both update and the execute are part of the Draw phase = 2, this ends up extending the draw animation duration as well, which will cause the pose animation frames to repeat itself as long the duration goes. Maybe the step animation for the Draw pose can be turned OFF here somehow, but I don't know how to do that without affecting the "executeDraw" function.
	} else if ($gameTemp._chrono.draw[2] === 1) {
		$gameTemp._chrono.draw[2] = 2;
		$gameTemp._chrono.draw[0] = false;
		$gameSystem._chronoMode.phase = 3;
	}
};

//==============================
// * Execute Kill Minions Phase - added by FlintX's brother. This particular new phase only triggers if an enemy has a special notetag that indicates if it is a boss, and if other enemies are minions of this stated boss.
//==============================
Game_Chrono.prototype.executeKillMinions = function(minions) {
	$gameSystem._chronoMode.phase = 11;
    $gameTemp._chrono.phase[0] = true;
	$gameTemp._chrono.phase[1] = 11;	
	$gameTemp._chrono.killminions[3] = minions;
	$gameSystem._chronoMode.phaseDuration = 60;
};

//==============================
// * Update Kill Minions Phase - added by FlintX's brother. The Update for Kill Minions Phase.
//==============================
Game_Chrono.prototype.updateKillMinions = function() {
	$gameTemp._chrono.phase[0] = false;
	$gameTemp._chrono.phase[1] = 11;
	if ($gameTemp._chrono.killminions[2] === 0) {
		$gameTemp._chrono.killminions[2] = 1;
		$gameTemp._chrono.killminions[0] = true;
		$gameTemp._chrono.killminions[1] = true;
		$gameSystem._chronoMode.phaseDuration = 30;
		//AudioManager.playSe(); //I don't think audio is needed for this, since evey single "minion" killed will play the "collapse" sound effect. Well, maybe can be useful for giving extra indication to what happened, but keep in mind this is a "global" sound for the entire "phase", not for each minion killed.
	} else if ($gameTemp._chrono.killminions[2] === 1) {
		$gameTemp._chrono.killminions[0] = false;

		const minions = $gameTemp._chrono.killminions[3];
		if (minions.length === 0) {
			$gameTemp._chrono.killminions[2] = 2;
		}
		else {
			minions.shift().setHp(0); // Removes first element
			$gameTemp._chrono.killminions[2] = 0;
		}
		this.executeDeadEffect();
	} else if ($gameTemp._chrono.killminions[2] === 2) {
		if (this.isAllEnemiesDead()) {
			this.executeThreatCleared();
		} else {
			$gameSystem._chronoMode.phase = 3;
			$gameTemp._chrono.killminions[0] = false;
			$gameTemp._chrono.killminions[1] = false;
			$gameTemp._chrono.killminions[2] = 0;
			$gameTemp._chrono.killminions[3] = [];
		}
	}
}
...
//==============================
// * Execute Threat Cleared Phase - FlintX added to create "lay down weapons" poses after battle is over.
//==============================
Game_Chrono.prototype.executeThreatCleared = function() {
	$gameSystem._chronoMode.phase = 8; //This value is tied to the pose code on my custom MOG_CharPoses.js
    $gameTemp._chrono.phase[0] = true;
	$gameTemp._chrono.phase[1] = 8;	//Since I have no ideia what those values means, I will keep it like that. It is working so far. I think as long as it doesn't use any value that is already tied to something else on MOG's own phases it is fine.
	$gameSystem._chronoMode.phaseDuration = 43; //Duration of "Sheath Pose" animation. This particular time was set for a 6 long frames animation. (Within Restart's CrossEngine Frame Counting behavior of course).
};

//==============================
// * update Threat Cleared - FlintX added for updating the new "Sheathing pose phase" into the Victory phase.
//==============================
Game_Chrono.prototype.updateThreatCleared = function() {
    $gameTemp._chrono.phase[0] = false;
	$gameTemp._chrono.phase[1] = 8;	//Since I have no ideia what those values means, I will keep it like that. It is working so far. I think as long as it doesn't use any value that is already tied to something else on MOG's own phases it is fine.
	if ($gameTemp._chrono.threatcleared[2] === 0) {
		$gameTemp._chrono.threatcleared[2] = 1;
		$gameTemp._chrono.threatcleared[0] = true;
		$gameTemp._chrono.threatcleared[1] = true;
		$gameSystem._chronoMode.phaseDuration = 30;
		//AudioManager.playSe(); //Can add sound effect for the Sheathing phase too if wanted or at least for when all enemies are dead!
	} else if ($gameTemp._chrono.threatcleared[2] === 1) {
		$gameTemp._chrono.threatcleared[2] = 2;
		$gameTemp._chrono.threatcleared[0] = false;
		
		let isBossFight = false; //bellow lines are meant for boss fights. When stated by notetags
		const enemies = $gameMap.enemiesF();
		for (var enemy_index = 0; enemy_index < enemies.length; enemy_index++) {
			const battler = enemies[enemy_index].battler();
			if (battler._ras._Boss[0]) {
				isBossFight = true;
				break;
			}
		}
		if (isBossFight) {
			this.executeBossVictory(); //execute the new Boss Victory phase if the battle is marked as a boss fight in the notes.
			return;
		}
		// Notes about updateThreatCleared:
		// I couldn't figure out a way to make this fall into the normal ($gameSystem._chronoMode.phase === 4)
		// and proceed naturally, so I forced the executeVictory function here. I'm not sure if 
		// this will break anything in the code, but as far as my tests went, it seems to conclude Chrono Battle System normally, 
		// and resumes standard gameplay without any issues, apparently. I mean, there is nothing else calling for the 
		// ($gameSystem._chronoMode.phase === 4), so I think it should be fine.
		this.executeVictory();  
	}
};

//==============================
// * Execute Counter Phase - FlintX not implemented ideia. Think of this as the original Chrono Trigger "counter" mechanic. 
//==============================
// Since the standard RPGMMV "Counter Mechanic" and "Reflect Magic" feature doesn't work in MOG's Chrono Battle System, I think this new phase could work as a placeholder for it. But due to how Chrono Mode was coded I don't see how that could be possible done without being hardcoded like this.
// In theory this phase would be called at the end of every "turnEndCN()" and executed whenever an enemy or player character takes damage by a Tool_ID or AbilityID used in battle. The phase would check for specific conditions in order to come into play.
// First it would check through the [StatesID] list and look if the attacked player character or enemy is afflicted by a [State] that would be carrying a "Counter State" notetag. (It means we could create Skills that would give this particular state in battle. In fact we could even force this state to be always ON for player characters whenever the [Guard Command] is selected. Which could make Guard command more useful in general).
// Second it would check if the attacked player character or enemy has a equipped [SkillID] carrying a "Counter" notetag. This would define with which [Skill] the player/enemy would use to counter attack.
// If all conditions are met, the ATB gauge would temporarily freeze/stop  execution of the corresponding Tool_ID would be FORCED into battle at that exact moment, and executed exactly as it is set in the Toll_ID Map, regardless if said enemy or player characters has their turns ready at ATB Gauge or not. (Pretty much how the counter works in Chrono Trigger).
// If none of those conditions are met, then the battle would resume on its standard behavior, calling the "_chronomode.phase 3 again". (which is the chronomode.phase where the main cycle of battle takes place I believe).
//Extra considerations:
// In additional to the core conditions above, the phase could also check for what was the HITTYPE_ of the Skill ID. And return a different counter for each. EX: A [Skill] with the "Counter Physical" notetag would counter HITTYPE_PHYSICAL, and one with "Counter Magic" would counter HITTYPE_MAGICAL.
// This also would allow to create different "Counter Stances" [States] that would trigger only for Physical, for Magical, or both HITTYPES. Ex: Make Counter Stance [States] that absorbs Physical/Magic hitype and heals the target, and then make Counters Attacks that will match the hitype received. With all those properties being defined in-RMMV Editor.

//==============================
// * Update Counter Phase - For the not implemented Counter Phase
//==============================
...
Game_Chrono.prototype.updatePhase = function() {
	...
	} else if ($gameSystem._chronoMode.phase === 13) {
		this.endChronoSystem2(); //FlintX added for the new event battles
	};
}; 

//==============================
// * check Battle Result - FlintX edited to fit the new "Threat Cleared/Sheathing" phase before vitory. //Update: My brother edited on this to fit all the new "Phases" created.
//==============================
Game_Chrono.prototype.checkBattleResult = function() {
	...
	if (this.isAllPartyDead()) {
		...
		/*if (this.isEventFight()) {
			$gameSystem._chronoMode.phase = 12; //Calling for the new phase 12 that later will fall into "pre End chrono system 2". It will skip victory/battle results phase and end Chrono mode battle system without giving XP/Gold. etc.
			$gameSystem._chronoMode.phaseDuration = 30; //edited to fit the new "Threat Cleared/Sheathing" phase (original value = 120)
			$gameSystem._chronoMode.phaseEndPhaseDuration = 30; //I'm not quite sure of what role this value serves. I made changes to it but it doesn't seems to affect anything in-engine at all. Maybe is how much time enemies sprites remains before disappearing??? (So I kept the original value anyway).
		} else {
			$gameSystem._chronoMode.phase = 8; //Changed value from 4 to 8. To fit the new "Threat Cleared/Sheathing pose" phase and the executeThreatCleared function.
			$gameSystem._chronoMode.phaseDuration = 44; //edited to fit the new "Threat Cleared/Sheathing" phase (original value = 120)
			$gameSystem._chronoMode.phaseEndPhaseDuration = 120; //I'm not quite sure of what role this value serves. I made changes to it but it doesn't seems to affect anything in-engine at all. Maybe how much time enemies sprites remains before disappearing??? (So I kept the original value anyway).
		}*/
		var eventFight = this.isEventFight(); //New - This searches for a notetag comment in the Enemies IDs to check if this is a "Event Fight". See the function for more info.
		if (eventFight) {
			this.executeEventFightVictory(); //New - Runs the new Event Fight phase "victory", which skips some elements from standard victory phases. See the actual function for more info.
		} else {
			this.executeThreatCleared(); //New - The new phase that plays just before the "Victory" phase, created for showing a "sheath/put down weapons" pose. 
		}
		this.clearCommandPhase();
		$gameSystem.clearChronoEscape();
		AudioManager.fadeOutBgm(2); //original value (3)
		clearParameters = true;
	} else if ($gameSystem._chronoMode.phase !== 11 ) { //New phase 11 only happens if the battle encounter is marked as a boss fight. In adition to all the "kill minions" stuff as well.
...
//==============================
// * execute Event Victory - FlintX "Event Victory" phase. The main purpose of this new phase is for creating battles that will do a smooth transition to the map events, so that you can have more flexibility to execute map events after a battle is over.
//==============================
Game_Chrono.prototype.executeEventFightVictory = function() {
	$gameSystem._chronoMode.phase = 12; //needed in order to make the "Sheath pose" animation plays at this moment as well.
	$gameTemp._chrono.phase[0] = true;
	$gameTemp._chrono.phase[1] = 12;	
	$gameSystem._chronoMode.phaseDuration = 44; //is how long the execution of this phase takes. Edited to fit this new phase (original value = 120). *Adjust at will to serve your needs.
	
};

//==============================
// * Update Event Victory - FlintX added the update for the new "Event Victory". It is basically the same code of "Update Victory" phase but with some changes to 
//==============================
Game_Chrono.prototype.updateEventFightVictory = function() {
	$gameTemp._chrono.phase[0] = false;
	$gameTemp._chrono.phase[1] = 12;
	if ($gameTemp._chrono.eventvictory[2] === 0) {
		$gameTemp._chrono.eventvictory[2] = 1;
		$gameTemp._chrono.eventvictory[0] = true;
		$gameTemp._chrono.eventvictory[1] = true;
		$gameSystem._chronoMode.phaseDuration = 30; //Due to my dumb code both the execute and update shares the same phase.value of 12. It means that 
		//$gameTemp._chrono.exp = this.resultExp();    //This, the bellow line and other line that controls XO and Gold gain is commented out so that we don't get any XP or Gold from this "Special Event Fight." See standard Victory phase and put the lines back ON if you want to receive them anyways. But keep in mind that there will be no screen telling the player that they have received any of that, since it has been removed from this particular phase chain.
		//$gameTemp._chrono.gold = this.resultGold();		
	} else if ($gameTemp._chrono.eventvictory[2] === 1) {
		$gameTemp._chrono.eventvictory[2] = 2;
		$gameTemp._chrono.eventvictory[0] = false;
		$gameSystem._chronoMode.phaseDuration = 0;	//edited to remove the last of the stepping animation after battle. This value controls the delay for followers to go back to the party leader after the final battle results screen. However it completely removes the waiting time before the battle results screens shows up. Not a big deal though since this particular screen requires an input in order to skip.
		BattleManager.replayBgmAndBgs();
	} else if ($gameTemp._chrono.eventvictory[2] === 2) {
		AudioManager.fadeOutMe(1);
		$gameSystem.onBattleWin();
		this.preEndChronoSystem2();			
	};	
};

//==============================
// * execute Victory - FlintX edited and moved the victory song audio to play at this moment.
//==============================
Game_Chrono.prototype.executeVictory = function() {
	AudioManager.playMe($gameSystem.victoryMe()); //removed from the update phase and put here to play the song as soon as the battle is won. You can also force a specific file Bgm to play here instead of the ".playMe(VictoryMe()) function as well, since audios of the Me class cannot loop apparently.
...
//==============================
// * update Victory - FlintX edited to get rid of steping animations just before followers reunites with leader. (Original value for the second $gameSystem._chronoMode.phaseDuration = 45).
//==============================
Game_Chrono.prototype.updateVictory = function() {
	...
	if ($gameTemp._chrono.victory[2] === 0) {
		...
		$gameSystem._chronoMode.phaseDuration = 30;
		//AudioManager.playMe($gameSystem.victoryMe()); // Moved to the execute phase instead.
...
//==============================
// * execute Boss Victory - FlintX addd to create a different victory phase fit to Boss Battles
//==============================
Game_Chrono.prototype.executeBossVictory = function() {
	$gameSystem._chronoMode.phase = 9;
	$gameTemp._chrono.phase[0] = true;
	$gameTemp._chrono.phase[1] = 9;	
	$gameSystem._chronoMode.phaseDuration = 90;
	
};

//==============================
// * update Boss Victory - FlintX replicated "Update Victory" phase code to create the "Update" equivalent for the new Boss Victory phase.
//==============================
Game_Chrono.prototype.updateBossVictory = function() {
    $gameTemp._chrono.phase[0] = false;
	$gameTemp._chrono.phase[1] = 9;	
	if ($gameTemp._chrono.bossvictory[2] === 0) {
		$gameTemp._chrono.bossvictory[2] = 1;
		$gameTemp._chrono.bossvictory[0] = true;
		$gameTemp._chrono.bossvictory[1] = true;
		$gameSystem._chronoMode.phaseDuration = 30;
		//AudioManager.playMe($gameSystem.victoryMe()); // Moved to the execute phase instead.
		$gameTemp._chrono.exp = this.resultExp();
		$gameTemp._chrono.gold = this.resultGold();		
	} else if ($gameTemp._chrono.bossvictory[2] === 1) {
		if (Input.isTriggered('ok') || TouchInput.isTriggered()) {
			this.gainGoldExp();
			$gameTemp._chrono.bossvictory[2] = 2;
			$gameTemp._chrono.bossvictory[0] = false;
			$gameSystem._chronoMode.phaseDuration = 0;	//edited to remove the last of the stepping animation after battle. This value controls the delay for followers to go back to the party leader after the final battle results screen. However it completely removes the waiting time before the battle results screens shows up. Not a big deal though since this particular screen requires an input in order to skip
			SoundManager.playCursor();
			BattleManager.replayBgmAndBgs();
		};
	} else if ($gameTemp._chrono.bossvictory[2] === 2) {
		AudioManager.fadeOutMe(1);
		$gameSystem.onBattleWin();
		this.preEndChronoSystem();			
	};
...
//==============================
// * pre End Chrono System                **Kalian: Possibility to add a switch call here or in end chrono system to call a common event with conditional branches for e
//==============================
...
//==============================
// * pre End Chrono System 2 - FlintX added for special event battles
//==============================
Game_Chrono.prototype.preEndChronoSystem2 = function() {
	for (var i = 0; i < $gameMap.players().length; i++) {
		 var char = $gameMap.players()[i];
	     var battler = char.battler();
		 battler.atbClearCN();
	...
	$gameSystem._chronoMode.needRemoveSprites = true;
	$gameSystem._chronoMode.needRemovehHuds = true;
	this.endChronoSystem2();
};

//==============================
// * end Chrono System 2 = FlintX added here for the new Event fights.
//==============================
Game_Chrono.prototype.endChronoSystem2 = function() {
	for (var i = 0; i < $gameMap.players().length; i++) {
		var char = $gameMap.players()[i];	
		char.nextX = null;
		char.nextY = null;
...
//==============================
// * load Battle Parameters - FlintX doubt (After reactivating the original "moveToTarget" code I have no ideia if "isDashing()" or "isJumping()" must be added on several lines here too, for whatever reasons. But I'm leaving this note as a reminder. Just in case it needs to be).
//==============================
...
//==============================
// * is All Enemies Dead - FlintX's brother added lines here to fit the new "Bosses" and "Minions" notetags, used to define boss fights.
//==============================
...
Game_Chrono.prototype.isAnyBossDead = function() { //New - For checking if bosses are dead.
	var dead = false;
	for (var i = 0; i < $gameMap.enemiesF().length; i++) {
		var battler = $gameMap.enemiesF()[i].battler();
		if (this.isBoss(battler) && battler.isDead()) {
		   dead = true;
		   break;
		};
   };
...
Game_Chrono.prototype.isBoss = function(battler) {
	return battler.isEnemy() && battler._ras._Boss[0]; //enemy must have 'Boss HP' tag
}

Game_Chrono.prototype.isBossDead = function(battler) { //New
	return this.isBoss(battle) && battler.isDead();
}

Game_Chrono.prototype.isAllBossesDead = function() { //New
	var dead = true;
	for (var i = 0; i < $gameMap.enemiesF().length; i++) {
	     var battler = $gameMap.enemiesF()[i].battler();
		 if (this.isBoss(battler) && !battler.isDead()) { dead = false; }
	};
	return dead;
};

Game_Chrono.prototype.getMinionsToKill = function() { //New - To search for and kill bosses' minions when the boss is killed
	let minions = [];
	const enemies = $gameMap.enemiesF().map(function(it) { return it.battler() });
	for (var i = 0; i < enemies.length; i++) {
		const enemy = enemies[i];
		const isBoss = enemy._ras._Boss[0];
		if (isBoss && enemy.isDead()) {
			const minionsToKill = this.getMinionsForBoss(enemy, enemies);
			if (minionsToKill.length > 0) {
				minions = minions.concat(minionsToKill);
			}
		};
	}
	return minions;
...
Game_Chrono.prototype.getMinionsForBoss = function(battler, enemies) { //New - To define which minions belongs to which boss. Stated by notetags.
	if (!this.isBoss(battler)) return [];

	const minions = [];
	for (var i = 0; i < enemies.length; i++) {
		const enemy = enemies[i];
		if (enemy.isDead()) continue;
		const isMinion = enemy._ras._Minion[0];
		if (isMinion) {
			const isMinionOfBoss = enemy._ras._Minion[1] === battler._ras._Boss[1];
			if (isMinion && isMinionOfBoss) {
				minions.push(enemy);
			}
		}
	}
...
//==============================
// * update - FlintX added with someone else help, the equivalent for the update Draw Phase and ThreatCleared.
//==============================
Game_Chrono.prototype.update = function() {
	...
	if ($gameSystem._chronoMode.phaseDuration > 0) {
		...
		if (!$gameSystem._chronoMode.inTurn) {
			if ($gameSystem._chronoMode.phase === 2) { //here the "updateDraw".
				this.updateDraw();
			} else if ($gameSystem._chronoMode.phase === 8) {
				this.updateThreatCleared(); //here the "updateThreatCleared".
			} else if ($gameSystem._chronoMode.phase === 11) {
				this.updateKillMinions(); 
			} else if ($gameSystem._chronoMode.phase === 12) {
				this.updateEventFightVictory();
			} else if ($gameSystem._chronoMode.phase === 5) {
				this.updateVictory();
			} else if ($gameSystem._chronoMode.phase === 9) { // added for the new "Boss Victory" phase.
				this.updateBossVictory();		
...
//==============================
// * move To Target CN - FlintX edited with help
//==============================
Game_Chrono.prototype.moveToTargetCN = function(char,battler,target,actionID) {
   ...
   if (this._toolTurn.position === TOOL_POSITION_DEFAULT) { //I don't know why but my brother created this "tool position default" constant and swapped the others ones for a similar thing, but I think it is because he thought it would be easier to adjust things this way. Coders will probably understand better.
	   char.moveToTarget(0,0,-4,target,false) //changed to reactivate move to target code instead of the jump to character one.
   }
};

//==============================
// * update Move ToTarget - FlintX obs*. I tried adding "!char.isDashing()" here for whatever reason and it caused the game to softlock after selecting the attack command with followers characters. Since I have no clue of what I'm doing in the first place I will leave this here as a reminder to myself. DON'T MESS AROUND WITH THE UPDATE FUNCTIONS BECAUSE IT WILL BREAK STUFF, ESPECIALLY THE ONES THAT INVOLVES THE TARGETING AND MOVEMENT. Unless you know exactly what you doing of course.
//==============================
...
//==============================
// * prepare Move Back CN - FlintX - Changes made by my brother here to fit the new "skill_target" tool_position. 
//==============================
Game_Chrono.prototype.prepareMoveBackCN = function(char,battler) {
   ...
   //the char.jumpToPositionIfNeeded below replaces the old:
   // if (this._toolTurn.position === 0) {
   //    char.jumpToPosition(char._chrono.orgX,char._chrono.orgY);
   char.jumpToPositionIfNeeded(this._toolTurn.position, char._chrono.orgX,char._chrono.orgY); //
   this.forceRestorePosition();
};

//==============================
// * update Move Back CN - - FlintX - A doubt here: Is adding isDashing() here for whatever reasons after changes made to moveToTarget necessary? I mean, I know dash count as movement anyways, but this is just a reminder in case it needs to be.
//==============================
...
//==============================
// * can Start TurnCN - FlintX added line here
//==============================
Game_Chrono.prototype.canStartTurnCN = function(battler) {
	 if ($gameSystem._chronoMode.phase === 11) return false; //if killing minions wait for the entire phase execution, so that the game doesn't soflock when imediately selecting commands. Also to prevent an already dead enemy to be target.
	 if ($gameSystem._chronoMode.inTurn) {return false};
	 if (!battler._chrono.action) {return false};
	 if (battler._chrono.inAction) {return false};
...
Game_Chrono.prototype.prepareTurnCN = function(battler,char) {
	 ...
	 battler._chrono.actionPhase = 0;
var actorID = char.battler()._actorId;
    $gameVariables.setValue(41, actorID);
	var actorIndex = $gameParty.members().indexOf(char.battler()) + 1; // Adding 1 to make it 1-indexed
	$gameVariables.setValue(42, actorIndex); // Store the actor's position in the party
  console.log("Position of the actor in the party: " + actorIndex);
console.log("ActorID of the current turn: " + actorID);
$gameSwitches.setValue(103, true);
};
...
//==============================
// * Update Crono - FlintX edited here once and added "!this.isDashing()" for whatever stupid reason that made me do it. But it broke either the updateForceMovementRas or setInitialActorsPositionChrono function for some reason. (Funny enough it sometimes caused the party leader character to move when starting a battle, which is already odd, since only the followers does move at all by default. The leader just walked away to very far locations on map from where the actual battle was happening. Causing the camera to be locked on him and preventing you from having sight of the battle at all. Until you attacked with said character to have the camera accompanying him). This glitch happened often when entering battles while dashing, and after having escaping a previous battle. This also only happened when approuching the fights from specific positions of the map. And was a complete gamebreaking thing at large maps. Very weird stuff indeed.
//==============================
...
//==============================
// * update Force Movement Ras - FlintX added here. Those codes were added here because I couldn't get this behavior to work on the proper chronomode phase
//==============================
Game_CharacterBase.prototype.updateForceMovementRas = function() {
	this._through = false;
	this._moveSpeed = 3;
	this._directionFix = false;
	if ($gameParty.members()) {this._dashing = true}; //Added by FlintX to make characters run to their initial battle positions instead of walking before the battle starts. However it causes followers to carry the dash pose after the fight ends, and perma enable dash while moving through the map for them. But luckly I figured out a way to "fix" that. By toggling it off after the battles are finished. *See the last code at end of this block.
    ...
    if (direction > 0) {this.moveStraight(direction)};
	    if (!this.isMoving()) {
			...
			if ($gameChrono.phase() === 1) {
				...
				if ($gameParty.members()) {this._dashing = false}; //added by FlintX to make followers characters not have dashing poses turned ON for forever after chrono mode battles ends. This behavior often happened when finishing battles with skills or magic.
				//Again, I have no clue if this is the correct way of making this behavior happen, since I'm no coder at all. But well, it works and also doesn't seem to break anything. So you got the point.
				//Remind: Perhaps this code could also be moved to the actual "preEndChronoSystem" or "endChronoSystem" instead. Adding this inside turn End CN function doesn't seem to complete solve the glitch either.//
			};
		};
};
...
//==============================
// * set Initial Position - FlintX added code line here for the new "skill_target" tool_position correctly track and execute the toolID action on the target location, instead of the user location. 
//==============================
ToolEvent.prototype.setInitialPosition = function() {
   if (this.user()._user.autoTarget && (this._tool.position === TOOL_POSITION_TARGET || this._tool.position === TOOL_POSITION_SKILL_TARGET)) { //line added here
...
ToolEvent.prototype.checkToolNotes = function() {
	if (!this._erased && this.page()) {this.list().forEach(function(l) {		
	       if (l.code === 108) {var comment = l.parameters[0].split(' : ')	
				...
				if (comment[0].toLowerCase() == "tool_position"){	//FlintX edit				
					...
					} else if (position === "skill_target") { //This was also previously added in CrossEngine.js too. But I think this is the one being used? I haven't tested to confirm this though.
		                this._tool.position = TOOL_POSITION_SKILL_TARGET;//added a new tool_position that works just like standard "Target", but that also has the jump effect while moving back to character original position after executing an action. Which is present in move_to_target original demo code.
						this._tool.autoTarget = true;
						//it doesn't ignores guard as standard "target" too.
					} else {
		                this._tool.position = 0;
					};
				};
				...
				/*if (comment[0].toLowerCase() == "tool_counterskill_id"){ //FlintX (not implemented ideia). The comment that would be add to the toolID map to define if the skill/tool will is a counter move.//
					var skill = $dataSkills[Number(comment[1])];
		            if (skill) {this._tool.skill = skill};
				};*/						
...
//==============================
// * move to Target - FlintX edited with Restart's code and added a line to make characters dash instead of walking.
//==============================
Game_Character.prototype.moveToTarget = function(x,y,hPlus,target,through) {  
   this._dashing = true;
   if (x === null) {return};
   if (y === null) {return};
   x+= target.battler()._ras.bodySize*((this.x>target.x)-(this.x<target.x))/2
   y+= target.battler()._ras.bodySize*((this.y>target.y)-(this.y<target.y))/2
...
Game_Character.prototype.jumpToPositionIfNeeded = function(position, x,y) { //Code added by FlintX's brother to adjust for the new tool_position. 
	if (position === TOOL_POSITION_DEFAULT || position === TOOL_POSITION_SKILL_TARGET)
		this.jumpToPosition(x,y);
}

//==============================
// * jump to Target - FlintX added with Restart's codes tweaks from his discussion with Uranium thread (For in-editor purposes only with script calls on ToolIDs. Doesn't affect Chrono Engine code directly) || It is basically a copied Move to Target code with some alterations. It also doesn't affect projectiles as "jumpToCharacter" does.
//==============================
Game_Character.prototype.jumpToTarget = function(x,y,hPlus,target,through) {
   var disX = target._x - this._x;
   var disY = target._y - this._y;
   disX+= target.battler()._ras.bodySize*((this.x>target.x)-(this.x<target.x))/2;
   disY+= target.battler()._ras.bodySize*((this.y>target.y)-(this.x<target.x))/2;
   if (!target && !this.battler()) {return};
   if (!target) {var target = this.getTargetCN()};
   if (!target) {return};
   if (!hPlus) {hPlus = 0};
   ...
   var preDirection = this._direction;
   var through = through != null ? through : false;
   /*var pos = this.isXYavailableMove(x,y,target,through);*/ //doing nothing here for the time being.
   this._through = false;
   this._direction = preDirection;
   this._directionFix = true;
   this.jumpexch(disX+x,disY+y,hPlus); //edited from original "Move to Target" to make offsets actually work here.
   this._directionFix = predirectionFix;
   this._through = prethrough;   
};
...
//==============================
// * enemy - FlintX my brother added for calling enemyIDs when doing script calls (it was supposed to be eventID though, since one battle can have multiple enemies with the same ID. This was my bad, sorry.)
//==============================
Game_Interpreter.prototype.enemy = function(id) {
	const enemies = $gameMap.enemiesF();
	for (var i = 0; i < enemies.length; i++) {
		const enemy = enemies[i].battler();
		if (enemy._enemyId === id) return enemies[i];
	}
     return null;
};
...
Spriteset_Map.prototype.refreshResultCR = function() {
	...
	$gameTemp._chrono.bossvictory[1] = false;
	this._resultCR.opacity = 0;
	this._resultCRExp.opacity = 0;
	this._resultCRGold.opacity = 0;
...
Spriteset_Map.prototype.updateResultCR = function() {
	if ($gameTemp._chrono.victory[1] || $gameTemp._chrono.bossvictory[1]) {this.refreshResultCR()};
	this._resultCRExp.opacity = this._resultCR.opacity;
	this._resultCRGold.opacity = this._resultCR.opacity;
	if ($gameTemp._chrono.victory[0] || $gameTemp._chrono.bossvictory[0]) {
